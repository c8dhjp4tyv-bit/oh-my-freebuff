 ...
 *
 * Non-editing (no write/edit tools), on a strong model. It has terminal access
 * for read-only inspection only — `git diff`, `git log`, running the tests — and
 * must not mutate the repo. Reports findings ranked by severity so the
...
How to review:
- Start from the diff. Use \`git diff\` / \`git status\` via run_terminal_command to see exactly what changed, then read the surrounding code for context. Use the terminal for read-only inspection and running tests ONLY — never modify files, checkout/reset, or install anything.
